...
function users_init(){
	...
	$paths = array(
		...
		# userPwd=123 // plain text, unsafe I know
		# userPwdAgain=123 // repeat it, just to be safe
...
function users_confirmSessionKey(){
	...
	$q = sprintf("SELECT * FROM sessions WHERE sessionKey = '%s' AND ip = '%s'", params_get('sessionKey', ''), $_SERVER['REMOTE_ADDR']);
