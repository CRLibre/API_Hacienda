...
- Additional native handlers are active for `XmlToBase64/encode`, `callback/callback`, `check/checkxml`, `ejemplo/hola|un_usuario`, `fileUploader/subir_certif|subir_xml|test`, `makeJson/makeJson`, `makeQR/makeQR`, `sendMail/sendmail`, `signXML/signFE`, and `version/version`.
- Remaining fallback-proxy modules are now only `crlibreall/*`, `geoloc/*`, and `wirez/*`.
