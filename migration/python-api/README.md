...
- `facturador/*` routes are dispatched in Python and tracked as in-progress migration; native partial handlers exist for `info`, `get_all_privinces`, `get_type_of_id`, `get_cantons`, `get_district`, `get_neighborhood`, `inser_to_log_table` while the rest still run in fallback-proxy mode.
