...
- `facturador/*` routes are dispatched in Python and tracked as in-progress migration; native partial handlers now cover catalog/log/config reads plus receiver/voucher/inventory and company/sucursal read lookups (`info`, `get_all_privinces`, `get_type_of_id`, `get_cantons`, `get_district`, `get_neighborhood`, `inser_to_log_table`, `getCompannyLocationInformation`, `get_companny_information(_admin)`, `company_get_env`, `get_*_credentials`, `get_tipo_impuesto`, `getUnid`, `get_active_receiver`, `get_receiver_by_id`, `get_vouchers`, `getProductByCode`, `get_inventory`, `getSucursales`, `getTerminales`, `getUsersCompanny`, `getUserPermissionById`, `companny_getMyInfo`, `companny_getMyConsecutive`), while the remaining routes still run in fallback-proxy mode.
